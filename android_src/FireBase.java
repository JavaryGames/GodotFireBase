...
public class FireBase extends Godot.SingletonBase {
	...
	public FireBase(Activity p_activity) {
		registerClass ("FireBase", new String[] {
			...
			"load_interstitial", "create_banner",
			//AdMob--
			...
			"crash_log_exception", "crash_log_warning", "crash_log_error"
			//Crashlytics--
		});
	...
	public void notifyInMinsWithTag (final String message, final int mins, final String tag) {
		activity.runOnUiThread(new Runnable() {
			public void run() {
				Notification.getInstance(activity).notifyInMinsWithTag(message, mins, tag);
			}
		});
	}
	...
	public void crash_log_exception(String message) {
		Crash.getInstance(activity).logException(message);
	}

	public void crash_log_error(String message) {
		Crash.getInstance(activity).logError(message);
	}

	public void crash_log_warning(String message) {
		Crash.getInstance(activity).logWarning(message);
	}
	//Crashlytics--
